# Robotics Learning Resources

Curated handouts, roadmaps, and reference guides for students learning robotics, maintained by Prof. Xian (Shawn) Li, Department of Electrical and Computer Engineering, Northeastern University.

These materials support self-study and the robotics courses taught in Northeastern ECE, including EECE 5550 Mobile Robotics, EECE 5554 Robotics Sensing and Navigation, and Assistive Robotics. Anyone is welcome to use them.

## Resources

| Resource | Format | Level | Original source |
|----------|--------|-------|-----------------|
| [Becoming a Robotics Engineer: A Six-Month Self-Study Roadmap](handouts/Robotics_Engineer_6_Month_Roadmap_Handout.pdf) | PDF handout | Beginner to intermediate | [Ronin (@deronin_) on X](https://x.com/deronin_/status/2095180126359105955) |

More resources will be added over time.

## About the handouts

**Six-Month Robotics Roadmap.** A step-by-step plan that starts from basic electronics and ends with robot learning and portfolio building. Each month includes curated free and low-cost resources, key concepts, practice projects, and a milestone checklist. Topics cover electronics and microcontrollers, CAD and 3D printing, ROS 2 and simulation, control and kinematics, perception, and imitation learning with the SO-101 arm. Prices and links were checked by the original author in September 2026.

## How to use these materials

**For self-study.** Work through a handout at your own pace. Focus on the practice tasks, since building and debugging real systems teaches more than reading about them.

**For coursework.** Use the relevant sections as background reading or as a refresher before a lab. Each handout notes which sections relate to specific courses.

**Keep a record of your work.** Put every project in its own GitHub repository with a README that includes a photo or video, a wiring or architecture description, your measured results, and a short section on what broke and how you fixed it.

## Repository structure

```
robotics-resources/
├── README.md
└── handouts/
    └── Robotics_Engineer_6_Month_Roadmap_Handout.pdf
```

## Credit and attribution

Some handouts here are adapted from work by other authors. Each adapted handout:

- names the original author and links to the original work on its first page and in its footer
- condenses and rewords the original for student use rather than reproducing it in full
- keeps credit for the original ideas, structure, and research with the original author

Readers should consult the original source for full explanations. If you are an original author and would like a change or removal, please open an issue or contact me directly.

## Suggestions and corrections

Links and prices change often. If you find a broken link, an outdated price, or a resource worth adding, please [open an issue](../../issues).

## License

Original material written for this repository is licensed under [CC BY-NC 4.0](https://creativecommons.org/licenses/by-nc/4.0/). You may share and adapt it for non-commercial purposes with credit. Commercial use requires written permission. See [LICENSE](LICENSE) for the full terms.

Content adapted from other authors remains credited to them, and their original works keep their own terms.

## Maintainer

Prof. Xian (Shawn) Li
Department of Electrical and Computer Engineering, Northeastern University
GitHub: [@ShawnLiNU](https://github.com/ShawnLiNU)
